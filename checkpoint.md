...
#### 性能与流畅度
- [ ] 粒子对象池：避免频繁创建/销毁爆发粒子
- [ ] 动画循环优化：减少 canvas 状态切换、按类型批量绘制
- [ ] 积雪层 offscreen 缓存，避免每帧重绘
- [ ] 高分屏/大屏适配：根据 DPR 和分辨率自动调整密度与大小

#### 视觉与交互体验
- [ ] 图案切换平滑过渡（旧图案渐隐、新图案渐显）
- [ ] 主题切换参数插值动画（密度/风力渐变）
- [ ] 积雪更真实：边缘融化快、鼠标凹槽回填、风吹波浪
- [ ] 鼠标漩涡手感优化：距离影响力度、避免粒子完全重叠

#### 代码质量
- [ ] 模块化 `snow.js`：拆分为 particle / drawers / effects / audio
- [ ] 配置集中化：颜色、速度、大小统一配置表
- [ ] 错误边界与健壮性：麦克风权限降级、异常值保护
